...
export default function HeroSection() {
  ...
  return (
    <div className="relative w-full h-[85vh] overflow-visible pb-32">
      {slides.map((s, i) => (
        <div key={i} className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ${i === current ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className="relative z-10 flex flex-col items-center justify-start h-full px-6 pt-16 text-center">
        ...
        <p className="text-white/80 mt-4 text-base md:text-lg max-w-2xl">{slide.sub}</p>
      </div>
    </div>
  )
}
